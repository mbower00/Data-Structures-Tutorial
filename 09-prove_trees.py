...
# NOTE: I did not do all the steps in the assignment. I also received some of advice and help from a classmate (outside of the team work time (which I did not end up attending))
...
class BST:
    ...
    def _insert(self, data, node):
        ...
        if data < node.data:
            ...
            # leave the function if the data is already present
            return 
        else:
    ...
    def _contains(self, data, node):
        ...
        """
        # BASE CASE 1 (return False): the current node is None
        if node == None:
            return False

        # BASE CASE 2 (return True): the current node's data is equal to the data
        elif data == node.data:
            return True
            ...
            if data < node.data:
                return self._contains(data, node.left) # pass the left of the node as the new node

            # RECURSION on the right side 
            else:
                return self._contains(data, node.right) # pass the right of the node as the new node
    ...
    def _traverse_backward(self, node):
        ...
        """
        if node != None: # do not continue if node is None
            yield from self._traverse_backward(node.right) # call _traverse_backward at the right side
            yield node.data # now its time to give this node's data
            yield from self._traverse_backward(node.left) # call _traverse_backward at the left side
    ...
    # Start Problem 4 #
    ###################
    def _get_height(self, node, height = 0):
        """
        ...
        get_height.
        """
        # BASE CASE 1: the current node is None
        if node == None:
            # return the height we have collected up to this point
            return height
            ...
            left_height = self._get_height(node.left, height)
            
            # recur on the right side
            right_height = self._get_height(node.right, height)

            # We return the higher of the two heights (or right if they are the same)
...
# Start Problem 5 #
###################
def _insert_middle(sorted_list, first, last, bst, added_indexes = [], is_first = True):
    """
    ...
    """
    # the first call: make the added_indexes list empty
    if is_first:
        added_indexes = []
    
    # get the middle index of the list, relative to the position we are in
    middle = ( (last - first) // 2 ) + first

    # BASE CASE: The relative middle has already been added OR middle is negative one
    if middle in added_indexes or middle == -1:
        return

    # print(sorted_list[middle])

    # insert data at the relative middle in the BST
    bst.insert(sorted_list[middle])
    
    # add the index to the added_indexes list
    added_indexes.append(middle)

    # copy added_indexes for passing
    added_indexes_copy = list(added_indexes)

    # get the new first and last for running in the lower half
    lower_first = first
    lower_last = middle-1

    # get the new first and last for running in the upper half
    upper_first = middle+1
    upper_last = last

    # run for the lower half
    _insert_middle(sorted_list, lower_first, lower_last, bst, added_indexes=added_indexes_copy, is_first=False)

    # run for the upper half
    _insert_middle(sorted_list, upper_first, upper_last, bst, added_indexes=added_indexes_copy, is_first=False)
